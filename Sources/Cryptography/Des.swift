...
public final class DesTransposer: EncryptTransposer {
	...
	public func transpose(data: Block, key: Block) throws -> Block {
		...
		for i in (0..<8).reversed() {
			let bits6 = udata & 0b111111
			udata >>= 6
			let x = (bits6 & 0b011110) >> 1
			let y = (bits6 & 0b100000) >> 4 | (bits6 & 0b1)
			let r = Self.SBox[i][Int(y)][Int(x)]
...
public final class DesExpander: KeyExpander {
	...
	static func KeyShift() -> [Int] {
		var key_shift = Array(repeating: 2, count: 16)
		key_shift[0] = 1
		key_shift[1] = 1
		key_shift[8] = 1
		key_shift[15] = 1
	...
	// shift 28 bit half to the left
	static func leftRotate(a: UInt64, shift: Int) -> UInt64 {
		if shift == 1 {
			let last = (a & (1 << 27)) >> 27
			return (a << 1) & ~(0b1 << 28) | last
		}
	...
	public func expandKey(key: Block) throws -> [Block] {
		...
		var right = ukey & 0xFFFFFFF
		var keys: [Block] = []
		for i in 0..<16 {
			left = Self.leftRotate(a: left, shift: key_shift[i])
			right = Self.leftRotate(a: right, shift: key_shift[i])
